      ...
      <div className="min-h-screen bg-gray-50">
        <div className="bg-white shadow-sm border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <form method="GET" action="/search.php" id="searchForm" className="flex flex-col md:flex-row gap-3">
            ...
            <div className="lg:w-3/4">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                ...
                <div className="flex items-center space-x-4 mt-4 md:mt-0">
                  <div className="flex items-center space-x-2 bg-gray-100 p-1 rounded-lg">
                    <a href={`/search.php${buildQuery({ ...filters, view: "grid", page: 1 })}`} className={`p-2 rounded ${viewMode === "grid" ? "bg-white shadow-sm" : "text-gray-500"}`} title="Grid View">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                      </svg>
                    </a>
                    <a href={`/search.php${buildQuery({ ...filters, view: "list", page: 1 })}`} className={`p-2 rounded ${viewMode === "list" ? "bg-white shadow-sm" : "text-gray-500"}`} title="List View">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <form method="GET" action="/search.php" className="flex items-center">
                    <input type="hidden" name="keyword" value={filters.keyword} />
                    <input type="hidden" name="location" value={filters.location} />
              ...
              {result.services.length === 0 ? (
                <div className="bg-white rounded-lg shadow-sm p-12 text-center">
                  ...
                  <div className="mt-6">
                    <a href="/search.php" className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">
                    ...
                    <div key={service.id} className={`bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border overflow-hidden ${viewMode === "list" ? "flex flex-col md:flex-row" : ""}`}>
                      ...
                      <div className={`p-4 flex-1 ${viewMode === "list" ? "md:border-l border-gray-200" : ""}`}>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <a href={`/service_detail.php?id=${service.id}`} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm">
                            Lihat Detail
                          </a>
                        </div>
                      </div>
              ...
              {result.totalPages > 1 && (
                <div className="flex justify-center mt-12">
                  <div className="flex items-center space-x-2">
                    {page > 1 && (
                      <a href={`/search.php${buildQuery({ ...filters, page: page - 1 })}`} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-500 hover:bg-gray-50 flex items-center">
                    ...
                    {Math.max(1, page - 2) > 1 && (
                      <>
                        <a href={`/search.php${buildQuery({ ...filters, page: 1 })}`} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">1</a>
                    ...
                    {Array.from({ length: Math.min(result.totalPages, page + 2) - Math.max(1, page - 2) + 1 }, (_, index) => Math.max(1, page - 2) + index).map((pageNumber) => (
                      <a key={pageNumber} href={`/search.php${buildQuery({ ...filters, page: pageNumber })}`} className={`px-4 py-2 rounded-lg ${pageNumber === page ? "bg-blue-600 text-white" : "border border-gray-300 text-gray-700 hover:bg-gray-50"}`}>
                        {pageNumber}
                      </a>
                    ))}
                    {Math.min(result.totalPages, page + 2) < result.totalPages && (
                      <>
                        ...
                        <a href={`/search.php${buildQuery({ ...filters, page: result.totalPages })}`} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">{result.totalPages}</a>
                      </>
                    )}
                    {page < result.totalPages && (
                      <a href={`/search.php${buildQuery({ ...filters, page: page + 1 })}`} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 flex items-center">
                        Selanjutnya
