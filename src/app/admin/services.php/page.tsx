      ...
      <DashboardShell role="admin" active="/admin/services.php" title="Admin Dashboard">
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
          <div className="p-5 border-b border-gray-100 flex justify-between items-center">
            ...
            <a href="/admin/service_add.php" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm"><i className="fas fa-plus mr-2" />Add Service</a>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {services.map((service) => (
                  <tr key={service.id}>
                    ...
                    <td className="px-6 py-4 text-sm font-medium"><div className="flex space-x-2"><a href={`/admin/service_detail.php?id=${service.id}`} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors" title="Detail"><i className="fas fa-eye" /></a><a href={`/admin/service_edit.php?id=${service.id}`} className="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 transition-colors" title="Edit"><i className="fas fa-edit" /></a><form action={deleteServiceAction}><input type="hidden" name="id" value={service.id} /><input type="hidden" name="return_to" value="/admin/services.php" /><ConfirmSubmitButton className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors" title="Hapus" message={`Yakin ingin menghapus layanan "${service.title}"? Tindakan ini tidak bisa dibatalkan.`}><i className="fas fa-trash" /></ConfirmSubmitButton></form></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
