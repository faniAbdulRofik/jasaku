                        ...
                        <tr key={category.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex justify-end space-x-2">
                              <a href={`/admin/category_edit.php?id=${category.id}`} className="text-blue-600 hover:text-blue-900 p-1 rounded hover:bg-blue-50 transition-colors" title="Edit">
                ...
                {result.totalPages > 1 && (
                  <div className="mt-6 flex items-center justify-between" data-aos="fade-up">
                    <div className="flex-1 flex justify-between sm:hidden">
                      {result.page > 1 && (
                        <a href={`/admin/categories.php${buildQuery({ page: result.page - 1, search })}`} className="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                          Sebelumnya
                        </a>
                      )}
                      {result.page < result.totalPages && (
                        <a href={`/admin/categories.php${buildQuery({ page: result.page + 1, search })}`} className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                          Selanjutnya
                        </a>
                      )}
                    </div>
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                      ...
                      <div>
                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                          {result.page > 1 && (
                            <a href={`/admin/categories.php${buildQuery({ page: result.page - 1, search })}`} className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                          ...
                          {Array.from({ length: lastPage - firstPage + 1 }, (_, index) => firstPage + index).map((pageNumber) => (
                            <a key={pageNumber} href={`/admin/categories.php${buildQuery({ page: pageNumber, search })}`} className={`${pageNumber === result.page ? "z-10 bg-blue-50 border-blue-500 text-blue-600" : "bg-white border-gray-300 text-gray-500 hover:bg-gray-50"} relative inline-flex items-center px-4 py-2 border text-sm font-medium`}>
                              {pageNumber}
                            </a>
                          ))}

                          {result.page < result.totalPages && (
                            <a href={`/admin/categories.php${buildQuery({ page: result.page + 1, search })}`} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
