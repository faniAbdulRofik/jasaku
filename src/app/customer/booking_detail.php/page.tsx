...
export default async function CustomerBookingDetail({ searchParams }: PageProps) {
  ...
  if (!booking || booking.customer_id !== session.id) redirect("/customer/bookings.php?error=Booking tidak ditemukan");
  ...
  return (
    <>
      ...
      <DashboardShell role="customer" active="/customer/bookings.php" title="Customer Dashboard">
        ...
        <BookingDetailView booking={booking} role="customer" backHref="/customer/bookings.php" />
      </DashboardShell>
      <SiteFooter />
    </>
  );
