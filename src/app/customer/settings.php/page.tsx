...
export default async function CustomerSettings({ searchParams }: PageProps) {
  const params = await searchParams;
  const session = await getSessionUser();
  if (!session || session.role !== "customer") redirect("/login.php?error=Anda harus login sebagai customer");
  ...
  return (
    <>
      ...
      <DashboardShell role="customer" active="/customer/settings.php" title="Customer Dashboard">
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
          <div className="p-5 border-b border-gray-100">
            <h1 className="text-lg font-semibold text-gray-800">Settings</h1>
          </div>
          <form action={saveProfileAction} className="p-6 space-y-6">
            <input type="hidden" name="return_to" value="/customer/settings.php" />
