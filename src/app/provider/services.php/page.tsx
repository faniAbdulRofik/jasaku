      ...
      <DashboardShell role="provider" active="/provider/services.php" title="Provider Dashboard">
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
          <div className="p-5 border-b border-gray-100 flex justify-between items-center">
            ...
            <a href="/provider/add_service.php" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm"><i className="fas fa-plus mr-2" />Tambah Layanan</a>
          </div>
                  ...
                  <tr key={service.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        <a href={`/service_detail.php?id=${service.id}`} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors" title="Lihat detail" aria-label={`Lihat detail ${service.title}`}>
                          <i className="fas fa-eye" />
                        </a>
                        <a href={`/provider/edit_service.php?id=${service.id}`} className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors" title="Edit layanan" aria-label={`Edit ${service.title}`}>
                          <i className="fas fa-pen-to-square" />
                        </a>
                        <form action={deleteServiceAction}>
                          <input type="hidden" name="id" value={service.id} />
                          <input type="hidden" name="return_to" value="/provider/services.php" />
                          <ConfirmSubmitButton
