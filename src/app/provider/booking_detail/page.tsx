...
export default async function ProviderBookingDetail({ searchParams }: PageProps) {
  ...
  if (!session || session.role !== "provider") redirect("/login?error=Anda harus login sebagai provider");

  const id = Number(single(params.id) || 0);
  const booking = await getBookingById(id);
  if (!booking || booking.provider_id !== session.id) redirect("/provider/bookings?error=Booking tidak ditemukan");

  return (
    <>
      <SiteHeader />
      <AlertMessages success={params.success} error={params.error} warning={params.warning} />
      <DashboardShell role="provider" active="/provider/bookings" title="Provider Dashboard">
        <div className="mb-6">
        ...
        <BookingDetailView booking={booking} role="provider" backHref="/provider/bookings" />
      </DashboardShell>
      <SiteFooter />
    </>
  );
