...
export default async function EditService({ searchParams }: PageProps) {
  ...
  if (!detail || detail.service.provider_id !== user.id) redirect("/provider/services.php?error=Layanan tidak ditemukan");
    ...
    <>
      ...
      <DashboardShell role="provider" active="/provider/services.php" title="Provider Dashboard">
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
          ...
          <ServiceForm categories={categories} service={detail.service} providerId={user.id} returnTo="/provider/services.php" />
        </div>
      </DashboardShell>
      <SiteFooter />
    </>
